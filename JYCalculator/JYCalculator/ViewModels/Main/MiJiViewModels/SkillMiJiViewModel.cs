...
namespace JYCalculator.ViewModels
{
    ...
    public class SkillMiJiViewModel : CollectionViewModel<MiJiViewModel>
    {
        ...
        [DoNotNotify]
        public bool HasQiPo => (SkillKey == "BY") && CheckedNames.Contains("BY_QiPo"); // 判断是否激活了气魄秘籍
        ...
        public SkillMiJiViewModel(JYDataBase jyDb, string skillkey) : this(jyDb.Recipe.RecipeMiJiViewModels[skillkey])
        {
        }

        public SkillMiJiViewModel(string skillkey) : this(StaticJYData.DB, skillkey)
        {
        }
        ...
        /// 从数据库中批量生成
        /// </summary>
        public static IDictionary<string, SkillMiJiViewModel> MakeViewModels(JYDataBase jyDb)
        {
            var res = jyDb.Recipe.RecipeMiJiViewModels.Keys.ToDictionary(skillkey => skillkey,
        ...
        {
            return MakeViewModels(StaticJYData.DB);
        }
